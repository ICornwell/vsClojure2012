Project:

	vsClojure

Description:

	A Visual Studio 2012 Extension for ClojureCLR. (ported from 2010 version)

Status:

	Release 1.1.0 available

Features:

	1.) Clojure project type
	2.) Building
	3.) Running
	4.) Syntax highlighting
	5.) Brace matching
	6.) Auto-indentation
	7.) File formatting (Editor menu command)
	8.) Integrated REPL
		* Start project REPL (Project menu command)
		* Load all project files into REPL (Project menu command)
		* Load active file into REPL (Editor menu command)
		* Switch to active file's namespace (Editor menu command)
		* History (CTRL + UP ARROW or CTRL + DOWN ARROW)
	9.) Block commenting

Installing:

	1.) Build and run the ClojureExtension.Deployment.vsix file in the ClojureExtension.Deployment\bin\Debug folder

Building:

	1.) Install the Visual Studio 2012 SDK.
	2.) Clone the vsClojure repository.
	3.) Open the vsClojure solution.
	4.) Build

License:

	See LICENSE.txt.

Attribution:

	See ATTRIBUTION.txt.